...
<%
                ...
                try{
                	...
                	String sql = "Select * from ServiceCustomer WHERE cid_encrypted='"+enc + "'";
  ...
  <nav class="navbar">
    <div>
      <h1 style="margin-top: -5px; margin-bottom: 0px; color: #4B0082; font-size: 32px;">Service Management System</h1>
    </div>
    <ul class="nav-links">
      ...
      <form action="Success1" method="post">
        <div class="profile-details">
          ...
          <table>
            ...
            <tr>
              <td><label>Phone:</label></td>
              <td><input type="phone" id="update-phone" name="num" maxlength="10" minlength="10" pattern="[0-9]{10}" style="height: 25px; border-radius: 4px;"></td>
            </tr>
            <tr>
              <td><label>Location:</label></td>
              <td><input type="text" id="update-location" name="addr" maxlength="100" onkeypress="return blockSpecialCharacters(event);" style="height: 25px; border-radius: 4px;"></td>
            </tr>
            <tr>
      ...
      <table id="myTable">
        ...
        List<Map<String,String>> dataInactive = new ArrayList<>();
                try{
                	...
                	con = DriverManager.getConnection(url,user,password);
                	stmt = con.createStatement();
                	String sql = "Select * from Services WHERE cid_encrypted='"+enc + "'";
                	rs = stmt.executeQuery(sql);
                	while(rs.next()){
  ...
  function deleteRow(serviceId) { 
	 // alert(serviceId);
	  if (confirm("Are you sure you want to delete this service?")) { 
		  window.location.href = 'delete.jsp?id=' + serviceId; 
	} 
	 }
