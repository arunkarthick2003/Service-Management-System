...
public class adminUpdate extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(!email.isEmpty()) {
                String UpdateQuery = "update ServiceCustomer set email=? where cid_encrypted='"+cid+"'";
            ...
            if(contact != "") {
                ...
                String UpdateQuery = "update ServiceCustomer set contact_num=? where cid_encrypted='"+cid+"'";
                PreparedStatement pstmt = con.prepareStatement(UpdateQuery);
                pstmt.setLong(1, contact_num);
                pstmt.executeUpdate();
            }
            if(address != "") {
                String UpdateQuery = "update ServiceCustomer set address=? where cid_encrypted='"+cid+"'";
                PreparedStatement pstmt = con.prepareStatement(UpdateQuery);
